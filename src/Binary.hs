...
run Binary{..} = Just $ withCurrentDirectory path $ withTempDir $ \tdir -> do
    ...
    system_ $ "cabal v2-sdist --output-directory=" ++ tdir
    let vname = name ++ "-" ++ ver
    ...
    tdirFiles <- getDirectoryContents tdir
    let [tdirTarball] = filter (".tar.gz" `isSuffixOf`) tdirFiles
    let tarballName = dropExtension $ dropExtension tdirTarball
    b <- withCurrentDirectory tdir $ do
        system_ $ "tar -xf " ++ tdirTarball
        withCurrentDirectory tarballName $ do
    ...
    if not b then
        ...
        copy (tdir </> tarballName </> "bin" </> zname) res

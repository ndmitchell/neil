...
checkGhci = do
    ...
    unless ("-W" `elem` src || all (`elem` src) ghcWarnings) $
        error $
            "The .ghci file does not enough of " ++ unwords ("-W" : ghcWarnings)
...
checkGithub tests = do
    ...
    unless (expect_tests `isInfixOf` src) $
        fail $
            "ghc: versions did not match " ++ show expect_tests
    ...
    forM_ xs $ \x -> do
        ...
        if bad == []
            then
                ...
                if (takeFileName x `elem` undocumented)
                    then
...
docWhitelist =
    ...
    , -- don't seem to end up present when exported through Shake on Travis
      "cmd"
    , "data CmdOption" -- reexported from shake through rattle, so lacks docs
    ]
...
withSDist no_warnings prefix run = withTempDir $ \tdir -> do
    ...
    when (bad /= []) $
        error $
            unlines $
                "The following files are not checked in, but are in the dist" : bad
...
run Sdist = Just $ do
    ...
    withSDist False "v1-" $ do
        ...
        systemLog_ $
            "cabal v1-configure --ghc-option=-Wunused-imports --disable-library-profiling "
                ++ "--ghc-option=-Werror "
                ++
...
run Docs{..} = Just $ do
    ...
    systemLog_ $
        "cabal v1-haddock --hoogle --html --hyperlink-source "
            ++ "--contents-location=/package/"
            ++ name
    withTempDir $ \dir -> do
...
fixFileLinks (stripPrefix "<a href=\"file://" -> Just xs)
  ...
  where
    dropHTML x = if takeFileName x == "html" then takeDirectory x else x
...
testedWith = do
    ...
    return $
        concat
            [ map f $ words $ map (\x -> if x == ',' then ' ' else x) $ drop 12 x
            | x <- lines src
            , "tested-with:" `isPrefixOf` x
            ]
  where
    f x
        | Just rest <- stripPrefix "GHC==" x = rest
...
checkReadme = do
    ...
    let badges =
            [ "[![Hackage version](https://img.shields.io/hackage/v/"
                ++ name
                ++ ".svg?label=Hackage)]"
                ++ "(https://hackage.haskell.org/package/"
                ++ name
                ++ ")"
            , "[![Stackage version](https://www.stackage.org/package/"
                ++ name
                ++ "/badge/nightly?label=Stackage)]"
                ++ "(https://www.stackage.org/package/"
                ++ name
                ++ ")"
            , "[![Build status](https://img.shields.io/github/actions/workflow/status/"
                ++ github
                ++ "/ci.yml?branch=master)]"
                ++ "(https://github.com/"
                ++ github
                ++ "/actions)"
            ]
    ...
    when (found < 2) $
        error $
            "Expected first line of README.md to end with at least 2 badges, got:\n" ++ unlines foundBadges ++ "Expected from:\n" ++ unlines badges
    when (found /= bangs) $
        error $
            "Unexpected badges, found " ++ show bangs ++ ", but only recognised " ++ show found

checkChangelog :: IO ()
checkChangelog = do
    res <-
        fromMaybe (error "Couldn't find changelog")
    ...
    when (missingDate /= []) $
        error $
            "Expected dates for all releases, but missing for " ++ show missingDate
...
checkPullRequestTemplate = do
    ...
    when (res /= required) $
        error $
            "Expected pull request template in the right form, but not found"
...
checkCabalFile = do
    ...
    let bad =
            ...
            , not $ any isSpace $ trim x
            , not $ "http" `isSuffixOf` x || "https" `isSuffixOf` x
            , not $ all (\x -> isLower x || x == '-') x
                ...
                ++ ["Invalid tested-with: " ++ show test ++ "\nShould be releases in of " ++ show ghcReleases | not $ validTests test]
...
validTests xs = xs `isInfixOf` reverse ghcReleases

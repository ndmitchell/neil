...
defAllow = ["7.0.4","7.2.2","7.4.2","7.6.3","7.8.2","7.8.3"]
...
checkCabalFile = do
    ...
    let bad =
            ...
            ["Invalid tested-with: " ++ show test | length test < 1 || not (null $ test \\ defAllow) || test /= reverse (sort test) || (not (test `isPrefixOf` reverse defAllow) && False)] ++
